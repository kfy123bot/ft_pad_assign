...
sub generate_apr_pdf {
    ...
    $self->_draw_center_info($page, $f_bold, $f_reg, $cx, $cy, $box_edge, $l_cnt, $b_cnt, $r_cnt, $t_cnt, \%data_by_side);
...
sub _draw_side_boxes {
    ...
    foreach my $pin (@$pins) {
        ...
        if ($num ne '-' && ($num == 1 || $num % 5 == 0 || $num eq '0')) {
            my $tn = $page->text(); $tn->font($font, $font_size); $tn->fillcolor('#000000');
            my ($nx, $ny);
        ...
        my $tl = $page->text(); $tl->font($font, $font_size); $tl->fillcolor('#000000');
        my ($tx, $ty, $rot) = (0, 0, 0);
...
sub _draw_header {
    ...
    # 主標題
    $txt->font($f_bold, 18); $txt->fillcolor('#000000');
...
sub _draw_center_info {
    my ($self, $page, $f_bold, $f_reg, $cx, $cy, $box_edge, $l, $b, $r, $t, $data) = @_;
    my $header = $self->{parser}->{header};
    ...
    my $step = $box_edge / ($final_max + 1);
    my $font_size = $step * 0.9;
    $font_size = 7 if $font_size > 7;
    $font_size = 2 if $font_size < 2;
    
    my $spacing = $font_size * 1.5;

    my $project = $header->{'PRODUCTION NO.'} || $header->{'PRODUCTION NO'} || "N/A";
    my $package = $header->{PACKAGE} || "N/A";
    my $version = $header->{VERSION} || "N/A";

    my $txt1 = $page->text();
    $txt1->font($f_reg, $font_size); $txt1->fillcolor('#000000');
    $txt1->translate($cx, $cy + $spacing);
    $txt1->text_center("Project: " . $project);

    my $txt2 = $page->text();
    $txt2->font($f_reg, $font_size); $txt2->fillcolor('#000000');
    $txt2->translate($cx, $cy);
    $txt2->text_center("Package: " . $package);

    my $txt3 = $page->text();
    $txt3->font($f_reg, $font_size); $txt3->fillcolor('#000000');
    $txt3->translate($cx, $cy - $spacing);
    $txt3->text_center("Version: " . $version);
}
