...
# 核心行為準則 (自 CLAUDE.md 整合)

## 1. 思考重於編碼 (Think Before Coding)
**不預設、不隱瞞疑慮、權衡取捨。**
在實作前：
- 明確陳述假設。若有不確定，務必詢問。
- 若存在多種解釋，請提出——不要默默選擇。
...
## 2. 簡約優先 (Simplicity First)
**用最少的程式碼解決問題。不寫推測性的程式碼。**
- 不添加要求之外的功能。
- 不為單次使用的代碼建立抽象。
- 不添加未被要求的「靈活性」或「可配置性」。
- 若 200 行代碼可以用 50 行實現，請重寫。

## 3. 外科手術式變更 (Surgical Changes)
**只觸碰必須觸碰的地方。只清理自己留下的變動。**
- 不「順便」改進相鄰代碼、註解或格式。
- 不重構未損壞的東西。
- 匹配現有風格，即使你有不同見解。
- 測試標準：每一行變更都應能直接追溯到使用者的要求。
